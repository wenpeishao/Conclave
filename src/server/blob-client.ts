...
export async function uploadBlob(httpBase: string, data: Uint8Array | string): Promise<BlobRef> {
...
export async function downloadBlob(httpBase: string, shaOrUri: string): Promise<Uint8Array> {
  const sha = shaOrUri.replace(/^conclave:\/\/blobs\//, "");
  const res = await fetch(`${httpBase.replace(/\/$/, "")}/blobs/${sha}`);
